 ...
 */
 void c44::clusterExtraction(pcl::PointCloud<pcl::PointXYZ>::Ptr sourceCloud,
                             // add list to hold array/linkedList of clusters
						     // NOTE: clusters are save as normal PCD files
						     // could just pass out cluster_indices
						     ...
						     int maxClusterSize)
{
	...
	// NOTE: cluster_indicies is a vector where each element of the vector is 
	//       a cluster. e.g. cluster_indicies[0] holds the first cluster,
	//       cluster_indicies[1] holds the second cluster.
	std::vector<pcl::PointIndices> cluster_indices;  // save array of indices
	pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec;
	ec.setClusterTolerance (clusterTolerance);
	ec.setMinClusterSize (minCluserSize);
