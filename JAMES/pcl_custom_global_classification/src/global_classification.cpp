...
segmentAndClassify (typename pcl::rec_3d_framework::GlobalNNPipeline<DistT, PointT, FeatureT> & global, std::string scene_path)
{
    ...
    reader.read(scene_path, *cloud_scene);
    cloud_scene->is_dense = false;
std::cout << "Reading Point Cloud, width: " << cloud_scene->width << " height: " << cloud_scene->height << " IsDense: " << cloud_scene->is_dense << std::endl;

    pcl::visualization::PCLVisualizer vis ("Global Classification");
    vis.setBackgroundColor(0, 0, 0);
    vis.addPointCloud<pcl::PointXYZRGB>(cloud_scene, "cloud_scene");
...
//std::cout << "Indicies recieved: " << indices.empty() << std::endl;
    ...
    for (size_t i = 0; i < clusters.size (); i++)
    {
        ...
        pcl::ScopeTime cluster_process_time ("Done catagorizing cluster ------ ");
//std::cout << "working on " << i << "th cluster" << std::endl;
      std::stringstream cluster_name;
      cluster_name << "cluster_" << i;
//std::cout << "creating random vis handler" << std::endl;
      pcl::visualization::PointCloudColorHandlerRandom<pcl::PointXYZ> random_handler (clusters[i]);
      //pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> random_handler(clusters[i], 200.0, 200.0, 200.0);
//std::cout << "adding point cloud with custom handler" << std::endl;
...
//std::cout << "grabbing cluster indicies" << std::endl;
      //pcl::PointIndices * cur_indi = indices[i].indices;
      std::vector<int> cluster_indi = indices[i].indices;
//std::cout << "setting cluster indicies begin: ";
//std::cout << *cluster_indi.begin() << " end: " << *cluster_indi.end() << std::endl;
...
main (int argc, char ** argv)
{
  ...
  if (desc_name.compare ("vfh") == 0)
  {
    ...
    segmentAndClassify<flann::L1, pcl::PointXYZ, pcl::VFHSignature308> (global, scene_path);
  }
  ...
  if (desc_name.compare ("cvfh") == 0)
  {
    ...
    segmentAndClassify<Metrics::HistIntersectionUnionDistance, pcl::PointXYZ, pcl::VFHSignature308> (global, scene_path);
  }
  ...
  if (desc_name.compare ("esf") == 0)
  {
    ...
    segmentAndClassify<flann::L1, pcl::PointXYZ, pcl::ESFSignature640> (global, scene_path);
  }
