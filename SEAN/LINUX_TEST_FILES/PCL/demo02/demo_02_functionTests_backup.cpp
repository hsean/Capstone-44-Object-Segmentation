...
/**
 * desc: This function takes the inliers and outliers of a plane and returns
 *       all points above the passed inliers as a point cloud.
 * param: (in) inliers - points contained in the plane
 *        (in) outliers - points outside of the plane
 ...
 */
void getPrism(pcl::PointCloud<pcl::PointXYZ>::Ptr inliers,
...
int main(int argc, char** argv)
{
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_planeInliers;
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_planeOutliers;             
	...
	time_before_execution = boost::posix_time::microsec_clock::local_time();  // time before
	result = c44::getPlane(cloud_original, cloud_planeInliers, cloud_planeOutliers, 
	                       0, 90, 1000, 0.01);
	time_after_execution = boost::posix_time::microsec_clock::local_time(); // time after
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_01.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_02.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_03.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_04.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_05.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_06.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_07.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_08.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	{
		difference = time_after_execution - time_before_execution;  // get execution time
		writer.write<pcl::PointXYZ> ("T04_planeInliers_09.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << std::setw(5) << difference.total_milliseconds() << ": "
	...
	if(getPlane(cloud_unaltered, cloud_planeInliers, cloud_planeOutliers, 
	            coefficients, indices, 0, 1.57, 1000, 0.01) < 0)  // default settings
	{ 
		std::cout << "ERROR: No plane found" << std::endl;
	}
	else
	{
		c44::getPrism(cloud_planeInliers, cloud_planeOutliers, cloud_objects);
		writer.write<pcl::PointXYZ> ("T05_objects_01.pcd", *cloud_filtered); // write out cloud
		#ifdef DEBUG
		std::cout << "plane segmentation, default: " 
		          << cloud_planeInliers->points.size() << " points" << std::endl;
		#endif
	}
	*/
...
int c44::getPlane(pcl::PointCloud<pcl::PointXYZ>::Ptr sourceCloud,  
                  ...
                  int ransacIterations, double thresholdDistance)
{
	// create objects for plane indicies and coefficients 
	...
	int i = 0, nr_points = (int) sourceCloud->points.size();
	while (sourceCloud->points.size() > 0.3 * nr_points)
	{
		...
		sourceCloud.swap(outliersCloud);
		++i;
	}
	return 0;
}
 ...
 * pre-cond: all output variables must be declared before function is called.
 */
void c44::getPrism(pcl::PointCloud<pcl::PointXYZ>::Ptr inliers,
                   pcl::PointCloud<pcl::PointXYZ>::Ptr outliers,                       
                   pcl::PointCloud<pcl::PointXYZ>::Ptr objectsCloud)
{
	...
	prism.setInputCloud(outliers);
	prism.setInputPlanarHull(cloudHull);
	prism.segment (*output);
	
	// get all points retrieved by the hull
	pcl::ExtractIndices<pcl::PointXYZ> ex;
	ex.setIndices(output);
	ex.filter(*objectsCloud);
}
