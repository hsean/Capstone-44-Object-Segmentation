...
public:
	...
	* pre-cond: organism must already be initialized, this includes
	*           adding and initializing parameters structure
	* post-cond: organism added to population. 
	* ret: 0 if organism added sucessfully,
	*     -1 if an organism already exists at index id
	*/
	int addOrganism(unsigned int id, COrganism* organism);
	...
	*/
	int removeOrganism(unsigned int id);
	...
	*/
	int displayOrganism(unsigned int id);
	...
	*     -1 if list is empty
	*/
	int saveOrganism(unsigned int id, std::string filename);
	...
	*     -1 if index is out of array bounds
	*/
	int setOrganismFitness(unsigned int id, int fitness);
	...
	*/
	int getOrganismFitness(unsigned int id);
	...
	*/
	int setOrganismParameters(unsigned int id, OrganismParameters* parameters);
	...
	*/
	OrganismParameters* getOrganismParameters(unsigned int id);
	...
	*/
	int setOrganismValid(unsigned int id, bool valid);
	...
	*      out of bounds
	*/
	bool getOrganismValid(unsigned int id);
	...
	*     -1 if id is out of array bounds
	*/
	int setOrganismExecutionTime(unsigned int id, int executionTime);
	...
	*/
	int getOrganismExecutionTime(unsigned int id);
...
private:
	...
	*       the higher the returned fitness value, the better the organism.
	...
	*/
	double calculateFitness(bool isValid, int executionTime);
};
