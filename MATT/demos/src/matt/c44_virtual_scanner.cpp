...
void saveCloud (const std::string &filename,
                const PointCloud<PointT> &output)
{
  TicToc tt;
  tt.tic ();
...
main (int argc, char** argv)
{
  ...
  for (int i = 0; i < number_of_points; i++)
  {
    ...
    for (size_t cp = 0; cp < cloud.points.size (); ++cp)
    {
      ...
      {
          // Gaussian
        case 1: { cloud.points[cp].x += gaussian_rng (); cloud.points[cp].y +=
          gaussian_rng (); cloud.points[cp].z += gaussian_rng (); break; }
      }
    }
    ...
    auto descriptor = model.computeDescriptor<c44::RigidBody::histogram_t>();
    saveCloud<c44::RigidBody::signature_t>(vfh_fname, *descriptor);
    cout << "hey pal" << endl;
