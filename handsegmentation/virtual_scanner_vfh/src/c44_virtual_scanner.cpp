// c44_virtual_scanner.cpp
//
// Created Apr 23, 2016
//
// Modified version of the pcl_virtual scanner; takes a .ply file, i.e.,
// the output of a CAD model viewer, and converts it into a series of .pcd
// files, from various viewpoints, and then also generates a view file
// histogram for each of these .pcds, for use in the matching stage of
// a 'global' 3D object recognition pipeline
//
// by default it generates 42 views of the model, and takes several minutes
// to run on a laptop from 2013
//
// see: http://robotica.unileon.es/mediawiki/index.php/PCL/OpenNI_tutorial_5:_3D_object_recognition_(pipeline)#Segmentation
...
main (int argc, char** argv)
{
  ...
  int subdiv_level = 1;
  double scan_dist = 3;
  std::string fname, base, vfh_fname;
  char seq[256];
  ...
  for (int i = 0; i < number_of_points; i++)
  {
    ...
    // Output has the PointNormal type in order to store
    // the normals calculated by MLS;
    // TODO: revisit whether moving least squares (MLS) is the best method;
    //   MLS was just the first technique I found that
    //   generated the needed type of cloud, i.e., PointNormal, as opposed to
    //   regular pcl::Normal; there might be a more intelligent way
    //   of doing this
    pcl::PointCloud<pcl::PointNormal> mls_points;
    pcl::PointCloud<pcl::PointNormal>::Ptr normals_ptr;
    ...
    normals_ptr.reset(new PointCloud<PointNormal>(mls_points));
    PointCloud<VFHSignature308> output;
    compute (normals_ptr, output);
    
    // Save into the second file
    saveCloud (vfh_fname, output);
